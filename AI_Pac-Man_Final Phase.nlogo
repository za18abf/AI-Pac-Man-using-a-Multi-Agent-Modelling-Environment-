...
;; existing code used- Wilensky, U. (1999). NetLogo. http://ccl.northwestern.edu/netlogo/. Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
;;;;;;;;;;;;;;;;;;;;;;;;
;;; Setup Procedures ;;;
;;;;;;;;;;;;;;;;;;;;;;;;
...
to new  ;; Observer Button
  ...
  set level 1
  load-map
  set score 0                   ;; declaring the initial variable value
  set lives 3
  set extra-lives 0
  set scared 0
...
to play  ;; Observer Forever Button
  ...
  if level-over?
  [ user-message word "Level Has Been Complete!\nScore: " score  ;; \n means start a new line
    set level level + 1
    load-map
    set level-over? false
    stop ]
  ...
  display
                                                                                                       ...
                                                                                                       ;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;AI Pacman Procedures ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                                                                                                       ;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; an intelligent AI Pacman program using multiagent search algorithm
;;my code implementation for the AI pacman procedure

to move-pacman  ;; Observer Procedure
  ask pacmans
  [ consume ;; consume command was called before the movement to avoid the glitch
    choose-heading-Pacman
    fd 1
    consume
...
to choose-heading-Pacman ;; pacman procedure
 let dirs clear-headings-Pacman
 let new-dirs-pacman remove opposite heading dirs
 let ghost-dir false
 let pellet-dir false
 if length dirs = 1           ;; an additional algorithm called multiagent search algorithm has been introduced here to make all the agents in game work together in sync
  [ set heading item 0 dirs ]
  if length dirs = 2
  [ ifelse see-ghost item 0 dirs
    [ set ghost-dir item 0 dirs ]
    [ ifelse see-ghost item 1 dirs
      [ set ghost-dir item 1 dirs ]
  [ ifelse see-pellets item 0 dirs
    [ set pellet-dir item 0 dirs ]
    [ ifelse see-pellets item 1 dirs
      [ set pellet-dir item 1 dirs]
      [ set heading one-of new-dirs-pacman ]
  ]]]]
  if length dirs = 3
  [ ifelse see-ghost item 0 dirs
    [ set ghost-dir item 0 dirs ]
    [ ifelse see-ghost item 1 dirs
      [ set ghost-dir item 1 dirs ]
      [ ifelse see-ghost item 2 dirs
        [ set ghost-dir item 2 dirs ]
  [ ifelse see-pellets item 0 dirs
    [ set pellet-dir item 0 dirs ]
    [ ifelse see-pellets item 1 dirs
      [ set pellet-dir item 1 dirs ]
      [ ifelse see-pellets item 2 dirs
        [ set pellet-dir item 2 dirs ]
        [ set heading one-of new-dirs-pacman ]
    ]]]]]]
  ...
  [ ifelse see-ghost item 0 dirs
    ...
    [ ifelse see-ghost item 1 dirs
      ...
      [ ifelse see-ghost item 2 dirs
        [ set ghost-dir item 2 dirs ]
        [ ifelse see-ghost item 3 dirs
          [ set ghost-dir item 3 dirs ]
   [ ifelse see-pellets item 0 dirs
    [ set pellet-dir item 0 dirs ]
    [ ifelse see-pellets item 1 dirs
      [ set pellet-dir item 1 dirs ]
      [ ifelse see-pellets item 2 dirs
        [ set pellet-dir item 2 dirs ]
        [ ifelse see-pellets item 3 dirs
         [ set pellet-dir item 3 dirs ]

          [ set heading one-of new-dirs-pacman ]
  ]]]]]]]]

  if ghost-dir != false;; targets the ghost when in scared state
  [ifelse scared > 0

    [ set heading ghost-dir ]
    [ set dirs remove ghost-dir dirs
     if see-ghost 0            ;; this may help the pacman avoid the ghost when they are not in scared state without random movements
      [set dirs remove 0 dirs]
      if see-ghost 90
      [set dirs remove 90 dirs]
      if see-ghost 180
      [set dirs remove 180 dirs]
      if see-ghost 270
      [set dirs remove 270 dirs]
      ifelse length dirs > 0
      [set heading one-of dirs]
     [ifelse length new-dirs-pacman > 0
     [set heading one-of new-dirs-pacman]
     [set heading one-of clear-headings-Pacman]


   ]
   ]
   ]
   ifelse pellet-dir != false
    [ set heading pellet-dir ]
    [ifelse length new-dirs-pacman > 0
    [set heading one-of new-dirs-pacman]
    [set heading one-of clear-headings-Pacman]]
...
to-report clear-headings-Pacman ;; pacman procedure;; this is used to check for the surrounding patch tiles of pacman for black patch and report it back
  let dirs []
...
to-report see-ghost [dir] ;; pacman procedure ;; a procedure to check and report the ghost in the map
  let saw-ghost? false
  let p patch-here
  ...
  [ ask p
    ...
    if p = patch-here [ report saw-ghost? ]
  ]
  report saw-ghost?
end

to-report see-pellets [dir] ;; pacman procedure ;; a procedure to detect and report the pacdots in the map
  let saw-pellet? false
  let p patch-here
  while [[pcolor] of p = black]
  [ ask p
    [ if any? pellets-here
    ...
    ;; stop looking if you loop around the whole world
    if p = patch-here [ report saw-pellet? ]
  ]
  report saw-pellet?
end
...
to return-home  ;; Ghosts Procedure
  let dirs clear-headings-Ghost
  let new-dirs-ghost remove opposite heading dirs
  let home-dir 0
  if pcolor != gray
    [ set home-dir towards one-of patches with [pcolor = gray] ]
  let home-path 90 * round (home-dir / 90)

  if length new-dirs-ghost = 1
  [ set heading item 0 new-dirs-ghost ]
  if length new-dirs-ghost > 1
  [ ifelse position home-path new-dirs-ghost != false
    [ set heading home-path ]
    [ set heading one-of new-dirs-ghost ]
  ]
end

to choose-heading-Ghost  ;; Ghosts Procedure
  let dirs clear-headings-Ghost
  let new-dirs-ghost remove opposite heading dirs
...
to-report clear-headings-Ghost ;; ghosts procedure
  let dirs []
...
to-report see-pacman [dir] ;; ghosts procedure ;; a function to detect pacman in the map
...
;; 02/02/2021 - the existing code was reviewed to see how the program works- Wilensky, U. (1999). NetLogo. http://ccl.northwestern.edu/netlogo/. Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
;; 04/02/2021 - an existing problem found within the code i.e,the pacman does not die sometimes when the ghost consumes pacman on a same path,this happens due to the difference in the speeds of the two agents.
;; 05/02/2021 - the problem found was fixed by calling the consume function twice before and after the movement of the pacman to avoid that glitch
;; 08/02/2021 - a random moving pacman program was created as a first step to get the pacman moving through the environment (first phase)
;; 10/02/2021 - for the second phase a pathfinding algorithm was researched to give a navigation control to the AI pacman to give awareness of the ghost in the environment and on how to avoid them
;; 23/02/2021 - the pathfinding algorithm was introducted in the program and worked successfully
;; 25/02/2021 - for the final phase multiagent search algorithm was researched to apply in the model. with this program the AI pacman will have full awareness of agents i.e; ghost and pacdots and will make them all work together seamlessly
;; 16/03/2021 - the multiagent search was applied into the program and working fine.
@#$#@#$#@
GRAPHICS-WINDOW
242
10
